...
def test_layernorm(dim=1024, eps=1e-5, dtype=torch.float16, bsz=21, random_state=3407, seqlen=3341):
    from torch.nn import LayerNorm
    layernorm = LayerNorm((dim,), eps=eps, device=device, dtype=dtype)
    torch.cuda.manual_seed(random_state)
    torch.manual_seed(random_state)
    torch.nn.init.uniform_(layernorm.weight)
    torch.nn.init.uniform_(layernorm.bias)
    X = torch.randn((bsz, seqlen, dim), dtype=dtype, device=device)
    XX = X.clone()
    X.requires_grad_(True)
    XX.requires_grad_(True)
    Y = layernorm(X)
    YY = torch.randn((bsz, seqlen, dim), dtype=dtype, device=device, requires_grad=True)
    Y.backward(YY)
