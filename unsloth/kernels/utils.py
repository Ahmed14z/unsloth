...
def fast_gemv(X, W, quant_state, out=None):
    ...
    fx(ctypes.c_int32(shape[0]), ctypes.c_int32(1), ctypes.c_int32(shape[1]), get_ptr(X), get_ptr(W), get_ptr(absmax), get_ptr(stats), get_ptr(out), ctypes.c_int32(shape[0]), ctypes.c_int32((X.shape[2] + 1) // 2), ctypes.c_int32(shape[0]), ctypes.c_int32(blocksize), CUDA_STREAM)
    return out
